...
graph TB
    ...
    subgraph ILOGYO_PROJECT["I.Log.Yo 프로젝트 (GCP)"]
        subgraph "GKE 클러스터"
            subgraph "인그레스"
                Ingress[Nginx Ingress]
            end
            
            subgraph "I.Log.Yo 네임스페이스"
                ILOGYO_FE[I.Log.Yo FE<br>React]
                ILOGYO_BE[I.Log.Yo BE<br>FastAPI]
                subgraph "I.Log.Yo BE 내 기능"
                    LOG_QUALITY[로그 품질 관리<br>통계 대시보드]
                    LOG_GUARD[로그 가드 기능<br>디버그/관리]
                end
            end
    ...
    ILOGYO_BE --> REDIS
    ILOGYO_BE <--> HACKLE
    ILOGYO_BE --> LOG_VALIDATOR
    
    EDW --> AIRFLOW
    AIRFLOW -- "Python Script로<br>Log Validator 로직 활용" --> REDIS
    AIRFLOW --> MART
    AIRFLOW --> POSTGRES
    
    LOG_QUALITY --> MART
    LOG_QUALITY --> POSTGRES
    ILOGYO_BE --> MART
...
1. **I.Log.Yo FE**
   ...
   - AB 테스트 설정 및 결과 분석 인터페이스

2. **I.Log.Yo BE**
   - FastAPI, Python 기반
   - 로그 스키마 관리 및 정의
   - Redis를 통한 로그 정의 캐싱
   - Hackle과 연동한 AB 테스트 설정 관리
   - 로그 품질 관리 메뉴 (Log Quality) 제공
   - 로그 가드 관리 메뉴 (Log Guard) 제공
...
4. **Log Validator**
   ...
   - 검증 결과를 Debug 앱에 응답하고 동시에 검증된 로그를 GCP Pub/Sub으로 전달
   - RT/Live 환경에서는 동일한 소스코드가 Airflow의 Python script로 활용됨

### 환경별 데이터 흐름
1. **Debug 환경**
   - 앱 → Log Collector → Log Validator → 검증 결과 앱으로 응답
   - Log Validator → GCP Pub/Sub → BigQuery(eDW) 
...
flowchart TB
    ...
    subgraph "처리 시스템"
        AIRFLOW[Airflow]
        LOG_VALIDATOR[Log Validator<br>FastAPI]
        VALIDATOR_SCRIPT[Log Validator<br>Python Script]
        PUBSUB[GCP Pub/Sub]
    end
    ...
    subgraph "관리 시스템"
        ILOGYO_FE[I.Log.Yo FE<br>React]
        ILOGYO_BE[I.Log.Yo BE<br>FastAPI]
        subgraph "I.Log.Yo BE 메뉴"
            LOG_QUALITY[로그 품질 관리]
            LOG_GUARD[로그 가드 관리]
        end
        HACKLE[Hackle<br>AB 테스트]
    end
    ...
    %% Debug 환경 데이터 흐름
    DEBUG_APP -->|실시간 로그| LOG_COLLECTOR
    LOG_COLLECTOR -->|검증 요청| LOG_VALIDATOR
    LOG_VALIDATOR -->|로그 정의 참조| REDIS
    LOG_VALIDATOR -->|검증 결과 응답| DEBUG_APP
    LOG_VALIDATOR -->|검증된 로그 전달| PUBSUB
    PUBSUB -->|로그 저장| EDW

    %% Live/RT 환경 데이터 흐름
    LIVE_APP -->|이벤트 데이터| GA4
    RT_APP -->|이벤트 데이터| GA4
    GA4 -->|10분 주기 적재| EDW
    ...
    ILOGYO_BE <-->|AB 테스트 설정| HACKLE
    
    %% 배치 처리 흐름
    EDW -->|로그 데이터| AIRFLOW
    AIRFLOW -->|로그 검증| VALIDATOR_SCRIPT
    VALIDATOR_SCRIPT -->|로그 정의 참조| REDIS
    VALIDATOR_SCRIPT -->|검증 결과| AIRFLOW
    AIRFLOW -->|통계 생성| MART
    ...
    %% 로그 가드 관리
    ILOGYO_FE -->|로그 가드 설정| LOG_GUARD
    LOG_GUARD -->|설정 저장| REDIS
```

### 데이터 플로우 설명

#### Debug 환경 데이터 흐름
1. Debug 앱에서 사용자 행동 로그를 Log Collector로 전송
2. Log Collector는 Log Validator에 정합성 검증 요청
3. Log Validator는 Redis에서 최신 로그 정의 정보를 참조하여 검증 수행
4. 검증 결과를 Debug 앱으로 실시간 응답
5. Log Validator는 검증된 로그 데이터를 GCP Pub/Sub으로 전달
6. GCP Pub/Sub은 수신한 로그를 BigQuery(eDW)에 저장
7. 개발자는 즉시 로그 정합성 피드백을 받고 수정 가능

#### Live/RT 환경 데이터 흐름
1. Live/RT 앱에서 사용자 행동 데이터를 GA4로 전송
2. GA4는 약 10분 주기로 데이터를 BigQuery의 eDW에 적재
3. Airflow는 스케줄에 따라 eDW에서 로그 데이터를 추출
4. Airflow에서 Log Validator의 로직을 Python script로 실행하여 로그 정합성 검증
5. 검증 결과와 함께 로그 품질 관련 통계 생성
6. 생성된 통계 데이터는 Data Mart(BigQuery)와 PostgreSQL에 모두 저장
...
3. Log Validator는 Redis에서 최신 로그 정의를 참조하여 검증 수행
4. I.Log.Yo BE는 Hackle과 연동하여 AB 테스트 설정 관리
...
1. **Debug 환경에서의 로그 검증 프로세스**
   ...
   - Log Validator는 검증 결과를 앱에 응답하고 검증된 로그를 GCP Pub/Sub으로 전달
   - GCP Pub/Sub을 통해 BigQuery에 로그 저장
   - 개발 단계에서 로그 정합성 문제를 조기에 발견하고 수정 가능

2. **로그 정의 관리 프로세스**
   - PO/개발자가 UI를 통해 로그 정의를 생성하고 관리
   - AB 테스트와 연동하여 사용자 행동 데이터 수집 설정
   - Redis 캐싱을 통한 성능 최적화
...
erDiagram
    ...
    HACKLE_AB_TEST {
        int id PK
        string event_id FK
        string test_id "AB 테스트 ID"
        string test_name "테스트 이름"
        json test_config "테스트 설정"
        date start_date "시작일"
        date end_date "종료일"
        string status "상태"
        datetime created_at
    ...
    LOG_DEFINITION ||--o{ HACKLE_AB_TEST : "AB 테스트"
...
#### HACKLE_AB_TEST (AB 테스트)
- Hackle과 연동된 AB 테스트 정보 관리
- 테스트 ID, 이름, 설정, 기간, 상태 등 기본 정보
- 특정 로그 이벤트와 연결된 AB 테스트 관리
- 테스트 결과 분석을 위한 기준 데이터 제공
